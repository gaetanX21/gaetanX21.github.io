...
Quite plainly, the goal of Sinkhorn's algorithm is to project a given square matrix $M\in\Rnn$[^positivity] onto the Birkhoff polytope

$$\B=\lbrace P\in\Rnn \mid P\one=\one, \one^T P=\one^T, P\geq 0\rbrace$$.
...
[^sparsity]: Here, "with sparsity" refers to NVIDIA's [structured sparsity](https://developer.nvidia.com/blog/structured-sparsity-in-the-nvidia-ampere-architecture-and-applications-in-search-engines/), a Tensor Core feature which essentially doubles your compute throughput if you have a 2:4 sparsity pattern, i.e. among each group of four contiguous values, at least two are zero. Since this feature effectively requires a 50% sparsity rate whereas neural network matrices are dense (unless you prune them for inference), I'm a bit skeptical regarding the relevance of this "with sparsity" metric.
